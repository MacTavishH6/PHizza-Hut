@extends('masterpage/masterpage')

@section('content_placeholder')
<div class="container" style=";width: 50%;margin-top: 50px">
    <div class="content" style="height: 100%;margin-top: 10px ">
        
        @foreach ($ChartList as $Data)
        <div class="UserGrid" style="outline-style: solid;outline-width: 0.5px;margin-bottom: 20px">
        <div class="row">
            <div class="col-sm-4">
                <img width="280px" height="280px" src="../storage/img/PHizzaHutLogo.jpg" style="padding: 50px 10px 10px 40px">
            </div>
            <div class="col-sm-8">
            <h2 style="margin-top:30px ">{{$Data->Pizza->PizzaName}}</h2>
            <p>{{$Data->Pizza->Price}}</p>
            <p>Quantity : {{$Data->PizzaQty}}</p>
            <form action="../UpdateChartQty/{{$Data->UserID}}/{{$Data->CartID}}" method="POST">
                {{ csrf_field() }}
                <span>Quantity: </span>
                <input style="margin-left: 200px;width: 300px" id="UpdateQty" type="number" name="UpdateQty" value={{$Data->PizzaQty}}>
                <br><br>
                <button type="submit" class="btn btn-primary" style="margin-bottom: 10px;width: 160px;color: white">Update Quantity</button>
                <br>
            </form>
            <a href="../DeleteCart/{{$Data->UserID}}/{{$Data->CartID}}" class="btn btn-danger" style="margin-bottom: 40px;width: 160px;color: white">Delete From Chart</a>
            
            </div>
        </div> 
        </div>
        @endforeach
        @if(count($ChartList) > 0)
        <div style="margin-left: 460px">
        <a href="../CheckOutPizza/{{$Data->UserID}}" class="btn btn-dark" style="margin-bottom: 40px;width: 160px;color: white;">Checkout</a>
        </div>
       
            @endif
    </div>
</div>
@endsection